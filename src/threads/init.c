...
pintos_init (void)
{
  ...
  if (*argv != NULL) {
    ...
    for(;;){
      printf("CS2043> ");
      int size = 100;
      char *input = malloc(size * sizeof(uint8_t));
      uint8_t chr;
      int length = 0;
      for(;;)
      {
      ...
      char whoamiC[] = "whoami";
      int whoamiC_b = strcmp(input,whoamiC);
      if (whoamiC_b == 0){
        printf("\nThemira Chathumina : 200088D\n");
        continue;
      }

      char shut_downC[] = "shutdown";
      int shut_down_b  = strcmp(input,shut_downC);
      if (shut_down_b == 0){
        printf("\nshutting down\n");
        shutdown_power_off();
        continue;
      ...
      char timeC[] = "time";
      int timeC_b  = strcmp(input,timeC);
      if (timeC_b == 0){
        time_t seconds = rtc_get_time();
        printf("\n%ld\n", seconds);
        continue;
      }

      char ramC[] = "ram";
      int ramC_b  = strcmp(input,ramC);
      if (ramC_b == 0){
        printf ("\n%d kB\n",
          init_ram_pages * PGSIZE / 1024);
        continue;
      ...
      char threadC[] = "thread";
      int threadC_b  = strcmp(input,threadC);
      if (threadC_b == 0){
        printf("\n");
        thread_print_stats();
        continue;
      ...
      char priorityC[] = "priority";
      int priorityC_b  = strcmp(input,priorityC);
      if (priorityC_b == 0){
        printf("\n%d\n",thread_get_priority());
        continue;
      }
      ...
      char sizeC[] = "size";
      int sizeC_b  = strcmp(input,sizeC);
      if (sizeC_b == 0){
        printf("\n%d\n",thread_get_priority());
        continue;
      }
      ...
      char exitC[] = "exit";
      int exitC_b  = strcmp(input,exitC);
      if (exitC_b == 0){
        printf("\nexiting shell\n");
        break;
      }
